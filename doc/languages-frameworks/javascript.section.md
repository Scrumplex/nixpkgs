...
stdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [
    nodejs
    pnpmConfigHook # this propagates `pnpm` but not `nodejs`!
  ];
...
+let
+  pnpm = pnpm_10;
+  # optionally, override nodejs as well:
+  # nodejs = nodejs_XX;
...
+in
 stdenv.mkDerivation (finalAttrs: {
   ...
   nativeBuildInputs = [
     nodejs
-    pnpmConfigHook # this propagates `pnpm` but not `nodejs`!
+    (pnpmConfigHook.override { inherit pnpm; }) # this propagates `pnpm` but not `nodejs`!
   ];
