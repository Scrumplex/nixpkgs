...
<!-- TODO: Is there a more user friendly way to display this change than using diff? -->
```diff
--- a/immich.sql
+++ b/immich.sql
...
`upload/upload/`. In order to replace just the prefix, and not all
...
from the brief time Immich was started on the system, we are going to
...
closely for any missing file paths until you are confident that
everything works.
